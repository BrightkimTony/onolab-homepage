...
- 2026-07-04: Contact 섹션을 더 전환형으로 강화했다. `무료 1차 채널 진단으로 시작합니다.`를 메인 문구로 쓰고, CTA는 `무료 진단 문의`로 변경했다. 사용자가 현재 운영 중인 채널 주소나 메뉴/상품 정보를 보내면 첫 점검 방향을 잡아준다는 안내 문구를 추가했다. 배포 커밋: `738b3d6 Strengthen consult conversion CTA`.
- 2026-07-04: 알로하오노/부산바다 서브도메인 HTTPS는 GitHub Pages 인증서 발급 대기 중이다. DNS/CNAME/CAA는 정상이며, Pages CNAME 재저장과 빈 커밋 재배포까지 진행했다. 재트리거 커밋: `alohaono-homepage` `87a5053`, `busansea-homepage` `b86eb5b`. 인증서가 뜨면 HTTPS enforcement를 켜면 된다. 자동 재확인 cron: `8a9a6a95-5d7d-4f70-8a21-d5d23e872342`.
